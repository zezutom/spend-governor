"""Sanity-check the cost computation against a known usage_metadata.

Run with: uv run python -m governor.cli.verify_cost
"""

from governor.pricing.cost import compute_baseline_llm_cost, token_usage_from_gemini
from governor.pricing.gemini import GEMINI_2_5_FLASH


SAMPLE_USAGE = {
    "prompt_token_count": 1953,
    "candidates_token_count": 31,
    "thoughts_token_count": 329,
}


def main() -> None:
    usage = token_usage_from_gemini(SAMPLE_USAGE)
    breakdown = compute_baseline_llm_cost(usage, GEMINI_2_5_FLASH)
    for key, value in breakdown.items():
        if isinstance(value, float):
            print(f"{key}: {value:.8f}")
        else:
            print(f"{key}: {value}")


if __name__ == "__main__":
    main()
